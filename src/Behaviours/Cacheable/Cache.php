...
class Cache
{
    ...
    {
        $this->model = $model;
        $this->configs = collect($model->caches())->map(function ($config) use ($model) {
    ...
    public function create() 
    {
        $this->apply(function ($config) {
            ...
            $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'created', $rawUpdate);
        });
    }
    ...
    public function delete() 
    {
        $this->apply(function ($config) {
            ...
            $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'deleted', $rawUpdate);
        });
    }
    ...
    public function update()
    {
        $this->apply(function ($config, $isRelevant, $wasRelevant) {
            ...
            $changedForeignKey = $this->model->getOriginal($foreignKey) && $this->model->{$foreignKey} != $this->model->getOriginal($foreignKey);
            ...
            switch ($config['function']) {
                case 'count': {
                    ...
                    if ($changedForeignKey) {
                        $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'updated', DB::raw("$summary + $value"));
                        $this->updateCacheRecord($this->model->getOriginal($foreignKey), $config, 'updated', DB::raw("$summary + (-1 * $value)"));
                        return;
                    }
                ...
                case 'sum': {
                    ...
                    if ($isRelevant && $wasRelevant) {
                        ...
                        if ($difference > 0) {
                            $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'updated', $difference);
                        }
                    } else if ($isRelevant && !$wasRelevant) {
                        // Increment because it was not relevant before but now it is
                        $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'updated', $value);
                    } else if (!$isRelevant && $wasRelevant) {
                        // Decrement because it was relevant before but now it is not anymore
                        $this->updateCacheRecord($this->model->{$foreignKey}, $config, 'updated', -1 * $originalValue);
                    }
    ...
    public function updateCacheRecord(
        ...
        $field = $config['field'];
        $key = $config['key'];
        $foreignKey = $foreignModel instanceof Model ? $foreignModel[$key] : $foreignModel; 

        $cacheQuery = $this->model::select(DB::raw("$function($field)"))->where($config['where'])->where($config['foreignKey'], $foreignKey);
        return $model::where($config['key'], $foreignKey)->update([
            $config['summary'] => $rawValue ?? DB::raw('(' . Cache::convertQueryToRawSQL($cacheQuery) . ')')
        ]);
    }
    ...
    public static function rebuildCacheRecords(
        ...
        $fields = collect([]);
        $updates = collect($foreignConfigs)->mapWithKeys(function ($configs, $foreignModel) use ($model, $fields) {
            return collect($configs)->map(function ($config) use ($foreignModel, $fields) {
                ...
                $query = $foreignModel::select(
                    DB::raw("$function($field)")
                )
                    ->where($config['where'])->where($config['foreignKey'], $model[$config['key']])->take(1);

                return [$config['summary'] => DB::raw('(' . Cache::convertQueryToRawSQL($query) . ')')];
            });
        })->toArray();
     ...
     * @return string
     */
    protected static function convertQueryToRawSQL(\Illuminate\Database\Eloquent\Builder $query)
    {
        $sql = $query->toSql();
